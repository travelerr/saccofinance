import {emailSecrets} from '@/lib/email/secrets';
import {emailDatabase} from '@/lib/email/database';
import {verifyResearchWebhook} from '@/lib/email/resend-webhook';
export const runtime='nodejs';
export async function POST(request:Request){
 let secret:string|undefined;
 try{secret=(await emailSecrets()).RESEND_WEBHOOK_SECRET;}catch{return new Response('Webhook unavailable',{status:503});}
 ...
 try{
  const {error}=await (await emailDatabase()).rpc('record_research_suppression',{p_event:event.id,p_type:event.type,p_hashes:event.hashes});
