...
export async function GET(request:NextRequest){
 ...
 if(token_hash&&client&&(type==='invite'||type==='recovery')){const{error}=await client.auth.verifyOtp({token_hash,type});if(!error)return NextResponse.redirect(new URL('/premium/set-password',process.env.AUTH_SITE_URL||request.url),{headers:{'Cache-Control':'private, no-store'}});}
