...
if(process.env.NEXT_PUBLIC_SUPABASE_URL!=='http://127.0.0.1:54321'||process.env.AUTH_SITE_URL!=='http://localhost:3082'||process.env.STRIPE_MODE!=='test'||!process.env.STRIPE_SECRET_KEY?.startsWith('sk_test_'))throw Error('Isolated local database and sandbox required');
const stripe=new Stripe(process.env.STRIPE_SECRET_KEY);const admin=createClient(process.env.NEXT_PUBLIC_SUPABASE_URL,process.env.SUPABASE_SECRET_KEY,{auth:{persistSession:false}});const origin='http://localhost:3082';const testPlan=process.env.BILLING_TEST_PLAN==='annual'?'annual':'monthly';const testPrice=process.env[testPlan==='annual'?'STRIPE_ANNUAL_PRICE_ID':'STRIPE_MONTHLY_PRICE_ID'];
