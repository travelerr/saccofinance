...
test('explicit local sandbox permits test keys but rejects live keys and hosted databases',()=>{const original={...process.env};try{Object.assign(process.env,{SACCO_LOCAL_DEVELOPMENT:'true',SACCO_LOCAL_STRIPE:'true',NEXT_PUBLIC_SUPABASE_URL:'http://127.0.0.1:54321',AUTH_SITE_URL:'http://localhost:3082',STRIPE_MODE:'test',STRIPE_SECRET_KEY:'sk_test_fixture',BILLING_SECRETS_ARN:'',EMAIL_SECRETS_ARN:'',RESEND_API_KEY:''});assert.equal(localDevelopment(),true);process.env.STRIPE_SECRET_KEY='sk_live_fixture';assert.throws(()=>localDevelopment());process.env.STRIPE_SECRET_KEY='sk_test_fixture';process.env.NEXT_PUBLIC_SUPABASE_URL='https://production.supabase.co';assert.throws(()=>localDevelopment());}finally{for(const key of Object.keys(process.env))if(!(key in original))delete process.env[key];Object.assign(process.env,original);}});
