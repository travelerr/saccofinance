...
function safeBaseEnv(){
 ...
 for(const key of ['SACCO_LOCAL_STRIPE','STRIPE_SECRET_KEY','STRIPE_WEBHOOK_SECRET','STRIPE_MONTHLY_PRICE_ID','STRIPE_ANNUAL_PRICE_ID','STRIPE_PORTAL_CONFIGURATION_ID','BILLING_SECRETS_ARN','BILLING_AWS_REGION','AWS_ACCESS_KEY_ID','AWS_SECRET_ACCESS_KEY','AWS_SESSION_TOKEN','AWS_PROFILE','EMAIL_SECRETS_ARN','EMAIL_AWS_REGION','RESEND_API_KEY','RESEND_WEBHOOK_SECRET','RESEARCH_EMAIL_ENABLED','RESEARCH_BROADCAST_ENABLED','RESEARCH_TEST_RECIPIENT','RESEARCH_EMAIL_FROM','SUPABASE_ACCESS_TOKEN','SUPABASE_DB_PASSWORD','OPENAI_API_KEY'])env[key]='';
...
function stripeConfig(){const file=path.join(localDir,'stripe-sandbox.json');if(!fs.existsSync(file))return {};const config=JSON.parse(fs.readFileSync(file,'utf8'));if(!config.STRIPE_SECRET_KEY?.startsWith('sk_test_')||!config.STRIPE_MONTHLY_PRICE_ID?.startsWith('price_')||!config.STRIPE_ANNUAL_PRICE_ID?.startsWith('price_')||!config.STRIPE_PORTAL_CONFIGURATION_ID?.startsWith('bpc_'))throw Error('Invalid local Stripe sandbox configuration');return Object.fromEntries(['STRIPE_SECRET_KEY','STRIPE_MONTHLY_PRICE_ID','STRIPE_ANNUAL_PRICE_ID','STRIPE_PORTAL_CONFIGURATION_ID','STRIPE_WEBHOOK_SECRET'].map(key=>[key,config[key]||'']));}
function appEnv(config){assertRuntime(config);const stripe=stripeConfig();return {...safeBaseEnv(),SACCO_LOCAL_DEVELOPMENT:'true',NEXT_PUBLIC_SUPABASE_URL:config.apiUrl,NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY:config.publishableKey,SUPABASE_SECRET_KEY:config.secretKey,AUTH_SITE_URL:'http://localhost:3082',NEXT_PUBLIC_SITE_URL:'http://localhost:3082',STRIPE_MODE:'test',STRIPE_BILLING_ENABLED:'true',RESEARCH_EMAIL_LOCAL_ENABLED:'true',EMAIL_SEND_MODE:'dry-run',RESEARCH_ADMIN_USER_IDS:config.adminId,...stripe,SACCO_LOCAL_STRIPE:stripe.STRIPE_SECRET_KEY?'true':'false'};}
...
module.exports={root,localDir,safeBaseEnv,appEnv,runtime,stripeConfig};
