export function safePremiumReturn(value:string|null|undefined):string{
 ...
 try{
  ...
  if(url.origin!=='https://return.invalid' || !url.pathname.startsWith('/premium/') || /^\/premium\/(login|signup|join|welcome|access|forgot-password|set-password|auth)(\/|$)/.test(url.pathname)) return '/premium/dashboard';
