...
export async function syncMemberBilling(userId:string){
 if(localDevelopment()&&!localStripeEnabled())return; // Read seeded payment evidence without contacting Stripe.
 ...
 for(const row of data){if(localStripeEnabled()&&row.stripe_subscription_id.startsWith('sub_local_'))continue;await reconcileBillingEvent({id:'account-sync-'+randomUUID(),type:'customer.subscription.updated',livemode:mode==='live',data:{object:{id:row.stripe_subscription_id}}} as unknown as Stripe.Event);}
}
